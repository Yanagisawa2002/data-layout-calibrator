...
task_stage="$task_root/build-stage-05"
mkdir "$task_stage"
...
cd "$task_root"
printf '%s\n' 'b0c75973de3ed05e78f36f2ccc2a1c0789df81c6782e05112e061a699cde251a  source-linux-05.tar.gz' | sha256sum --check
mkdir source-linux-05
tar -xzf source-linux-05.tar.gz -C source-linux-05
...
python3 Tools/WholeTask/build_native.py --output "$task_root/native-build-03"
...
command=json.load(open(root/'native-build-03/original-aos/command.json'))['command']
...
python3 Tools/WholeTask/linux_stage.py --output "$task_root/smoke-resource-02" --cpu-file "$task_cpu" --growth-gib 1 --timeout 600 -- python3 Tools/WholeTask/campaign.py --phase correctness --build "$task_root/native-build-03" --cases Tools/WholeTask/cases-smoke.json --output "$task_root/correctness-smoke-02"
python3 Tools/WholeTask/linux_stage.py --output "$task_root/discovery-correctness-resource-01" --cpu-file "$task_cpu" --growth-gib 1 --timeout 1200 -- python3 Tools/WholeTask/campaign.py --phase correctness --build "$task_root/native-build-03" --cases Tools/WholeTask/cases-discovery.json --output "$task_root/correctness-discovery-01"
cat > "$task_root/physics-cases-01.json" <<'JSON'
[{"id":"physics-long","resolution":24,"steps":4000,"exportEvery":1000,"re":0.1,"phase":0.0}]
JSON
python3 Tools/WholeTask/linux_stage.py --output "$task_root/physics-resource-01" --cpu-file "$task_cpu" --growth-gib 1 --timeout 1800 -- python3 Tools/WholeTask/campaign.py --phase correctness --build "$task_root/native-build-03" --cases "$task_root/physics-cases-01.json" --output "$task_root/correctness-physics-01"
